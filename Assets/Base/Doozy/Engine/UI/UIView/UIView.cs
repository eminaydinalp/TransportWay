...
namespace Doozy.Engine.UI
{
    ...
    public class UIView : UIComponentBase<UIView>
    {
        ...
        {
            var go =
                new GameObject(MenuUtils.UIView_GameObject_Name, typeof(RectTransform), typeof(UIView), typeof(Image));
        ...
        public void Show(bool instantAction = false)
        {
            ...
            if (!ShowBehavior.Animation.Enabled && !instantAction)
            {
                DDebug.Log(
                    ...
                    ") UIView, but you did not enable any SHOW animations. Enable at least one SHOW animation in order to fix this issue.",
                    this);
...
//            RemoveNullChildUIButtons();
        ...
        private void OnOrientationChange(DetectedOrientation newDeviceOrientation)
        {
                ...
                TargetOrientation == TargetOrientation.Landscape)
            {
                ...
                (TargetOrientation ==
                 TargetOrientation.Any) //if this UIView targets both landscape and portrait orientations)
            {
                ShowView(ViewCategory, ViewName); //show all the views that have the same 'viewCategory' and 'viewName'
            }
        ...
        private void ShowSelectDeselectButton()
        {
            if (AutoSelectButtonAfterShow &&
                SelectedButton !=
                null) //check that the auto select option is enabled and that a selectedButton has been referenced
...
//                    yield return new WaitForSecondsRealtime(HideBehavior.Animation.TotalDuration + UIViewSettings.DISABLE_WHEN_HIDDEN_TIME_BUFFER); //wait for seconds realtime (ignore Unity's Time.Timescale)
        ...
        {
            while (OrientationDetector.CurrentOrientation ==
                   DetectedOrientation.Unknown) //if the current orientation is unknown
            {
                OrientationDetector
        ...
        private static void ExecuteShow(string viewCategory, string viewName, bool instantAction)
        {
            ...
            foreach (UIView view in Database)
            {
                ...
                if (OrientationDetector.CurrentOrientation ==
                    DetectedOrientation.Unknown) //check that we do not have an Unknown orientation
                    OrientationDetector
