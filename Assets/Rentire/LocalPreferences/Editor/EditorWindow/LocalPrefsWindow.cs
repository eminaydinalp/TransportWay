...
namespace Neonagee.EditorInternal
{
    internal sealed class LocalPrefsWindow : LocalPrefsWindowBase
    {
        ...
        int selectedFile
        {
            get { return m_selectedFile = Mathf.Clamp(m_selectedFile, 0, files.Length - 1); }
        ...
        private void OnDisable()
        {
            ...
            {
                if (files != null && files.Length > selectedFile)
        ...
        public struct PrefGUI<T> : IPrefGUI
        {
            public struct Changes
            {
                ...
                public Changes(string CurrentKey, string NewKey, T Value, bool KeyChanged, bool ValueChanged,
            ...
            public Prefs<T> DoLayout(Prefs<T> prefs)
            {
                ...
                if (GUILayout.Button(new GUIContent("", !searching ? "Clear All" : "Remove Found Items"),
                        "WinBtnCloseMac"))
                {
                    if (!searching)
                        prefs.ClearAll();
                ...
                if (EditorGUILayout.BeginFadeGroup(collapse.faded))
                {
                    if (prefs.Count > 0)
                    {
                        ...
                        foreach (var value in prefs.dictionary)
                        {
                            ...
                            if (type == typeof(bool))
                                newValue = GUILayout.Toggle(newValue, newValue ? "True" : "False",
                            ...
                            if (keyChanged || valueChanged || remove)
                                changes.Add(new Changes(value.Key, newKey, newValue, keyChanged, valueChanged, remove,
                                    i));
                            i++;
                        }
                    }
        ...
        public void OnGUI()
        {
            ...
            if (EditorGUILayout.DropdownButton(
                    !Application.isPlaying
                        ? new GUIContent(" " + files[selectedFile] + " ")
                        : new GUIContent(" " + LocalPrefs.currentFile + " "),
                    FocusType.Passive, EditorStyles.toolbarDropDown))
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    menu.AddItem(new GUIContent("Open Files Folder"), false,
                        () =>
                        {
                            EditorUtility.RevealInFinder(script.FilesPath + files[selectedFile] +
                                                         LocalPrefs.filesExtension);
                        });
                }
                else
                {
            ...
            if (GUILayout.Button(GUIContent.none, EditorStyles.toolbarDropDown, GUILayout.Width(17)))
            {
                ...
                {
                    if (EditorUtility.DisplayDialog("Delete " + files[selectedFile],
